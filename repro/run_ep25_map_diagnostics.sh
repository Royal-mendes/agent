...
BAG=$BASE/apexnav_ep25_diagnostics.bag
log_stage "start diagnostic rosbag"
nohup bash -lc "source /opt/ros/noetic/setup.bash; export ROS_MASTER_URI=http://localhost:11311; exec rosbag record -O '$BAG' /grid_map/value_map /grid_map/free /grid_map/unknown /grid_map/occupied /grid_map/occupied_inflate /planning_vis/frontier /planning_vis/viewpoints /planning_vis/topo_path /robot /ros/expl_result /ros/expl_state /habitat/plan_action" \
  > "$LOGDIR/rosbag_diagnostics.log" 2>&1 &
BAG_PID=$!
sleep 2
...
RGB_VIDEO=$(find /root/autodl-tmp/ApexNav/agent-Apexnav/videos -maxdepth 1 -type f -name "*.mp4" -newer "$BASE/start.stamp" -printf "%T@ %p\n" 2>/dev/null | sort -n | tail -n 1 | cut -d" " -f2-)
if [ -z "$RGB_VIDEO" ]; then
  RGB_VIDEO=$(find /root/autodl-tmp/ApexNav/agent-Apexnav/videos -maxdepth 1 -type f -name "*.mp4" -printf "%T@ %p\n" 2>/dev/null | sort -n | tail -n 1 | cut -d" " -f2-)
fi
if [ -n "$RGB_VIDEO" ]; then
  cp "$RGB_VIDEO" "$BASE/videos/ep_0025_couch_rgb.mp4"
fi
log_stage "RC=$RC"
log_stage "RGB_VIDEO=$RGB_VIDEO"
log_stage "BAG=$BAG"

log_stage "render separate map diagnostics"
bash -lc "source /opt/ros/noetic/setup.bash; exec /usr/bin/python3 /root/autodl-tmp/ApexNav/agent-Apexnav/repro/render_apexnav_map_diagnostics.py --bag '$BAG' --out-dir '$BASE/videos'" | tee -a "$BASE/run.log"

echo "$BASE" > /root/autodl-tmp/ApexNav/agent-Apexnav/repro/latest_map_diagnostics_ep25_dir.txt
exit "$RC"
