...
LOG_DIR="$ROOT_DIR/repro/logs/benchmark_${DATASET}_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
cd "$ROOT_DIR"
...
case "$DATASET" in
  hm3dv1|hm3dv2|mp3d) ;;
  *)
    echo "Usage: $0 {hm3dv1|hm3dv2|mp3d}" >&2
    exit 2
    ;;
esac
...
python habitat_evaluation.py --dataset "$DATASET" need_video="$NEED_VIDEO" \
