    ...
    const std::string& selected_skill, const Vector3d& pos, const double& yaw)
{
  if (selected_skill.empty() || selected_skill == "FALLBACK_APEXNAV" ||
      selected_skill == "FOLLOW_APEXNAV_PROPOSAL" ||
      selected_skill == "VERIFY_TARGET" ||
      selected_skill == "NAVIGATE_TO_CONFIRMED_TARGET") {
    if (selected_skill == "FOLLOW_APEXNAV_PROPOSAL")
      ROS_WARN("[Reflective Skill] FOLLOW_APEXNAV_PROPOSAL via original ApexNav policy");
  ...
  if (selected_skill == "RECOVER_FROM_STUCK") {
    ROS_WARN("[Reflective Skill] RECOVER_FROM_STUCK via ApexNav fallback recovery");
    return planNextBestPoint(pos, yaw);
  }
