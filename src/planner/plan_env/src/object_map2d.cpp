...
void ObjectMap2D::getObjects(vector<vector<Eigen::Vector2d>>& clusters,
    vector<Vector2d>& averages, vector<int>& labels, vector<double>* confidences,
    vector<int>* observation_nums)
{
  clusters.clear();
  averages.clear();
  labels.clear();
  ...
  for (auto object : objects_) {
    ...
    if (object.best_label_ >= 0 &&
      ...
      confidence = object.confidence_scores_[object.best_label_];
      observation_num = object.observation_nums_[object.best_label_];
    }
    if (confidences)
      confidences->push_back(confidence);
    if (observation_nums)
      observation_nums->push_back(observation_num);
  }
}
